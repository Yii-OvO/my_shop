...
type GoodsOptionsCommonAddUpdate struct {
	GoodsId uint   `json:"goods_id"  description:"主商品id"    v:"required#主商品id必传"`
...
type GoodsOptionsDeleteReq struct {
	g.Meta `path:"/goods/options/delete" method:"delete" tags:"商品规格" summary:"删除商品规格"`
	Id     uint `v:"min:1#请选择需要删除的商品规格" dc:"商品规格id"`
}
type GoodsOptionsDeleteRes struct{}

type GoodsOptionsUpdateReq struct {
	g.Meta `path:"/goods/options/update" method:"post" tags:"商品规格" summary:"修改商品规格"`
	Id     uint `json:"id" v:"required#商品规格id不能为空" dc:"商品规格id"`
	GoodsOptionsCommonAddUpdate
}
...
type GoodsOptionsGetListCommonReq struct {
	g.Meta `path:"/goods/options/list" method:"get" tags:"商品规格" summary:"商品规格列表"`
	CommonPaginationReq
}
